...
export function resolveConfig(raw) {
  ...
  if (rawCurrency !== undefined && (typeof rawCurrency !== 'string' || !/^[A-Za-z]{3}$/.test(rawCurrency))) {
    throw new Error('token-fee: displayCurrency 必须是三位货币代码，例如 CNY')
  }
  ...
  return {
    displayCurrency: rawCurrency === undefined ? DEFAULT_CURRENCY : rawCurrency.toUpperCase(),
    pricingFile: file,
...
export function createProjectionDefinition(store) {
  return {
      ...
      parse(value) {
        ...
        return {
          ...
          last: last === null || typeof last !== 'object'
            ? null
            : { turn: Number(last.turn), step: Number(last.step), key: String(last.key) },
        }
      },
    },
    ...
    apply: (state, event) => {
      ...
      const previousKey = state.last !== null && state.last.turn === turn && state.last.step === step
        ? state.last.key
        : null
      const previous = previousKey === null ? undefined : state.rows[previousKey]
      const rows = { ...state.rows }
      if (previousKey !== null && previous !== undefined) {
        const cleared = subtractBuckets(previous, previous)
        const remaining = addBuckets(previous, cleared)
        if (isZeroBuckets(remaining)) delete rows[previousKey]
        else rows[previousKey] = remaining
      }
      ...
      return { route: state.route, rows, last: { turn, step, key } }
    },
    wire: {
      viewSchema: {
        parse(value) {
