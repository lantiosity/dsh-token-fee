...
/** 投影折叠语义的版本；折叠逻辑变化时必须递增。 */
const FOLD_VERSION = 3

/** 表指纹的进位基数，保证 `FOLD_VERSION * REVISION_SCALE + 指纹` 仍是安全整数。 */
const REVISION_SCALE = 1e9

/**
 * 有效价目表的内容指纹（32 位）。
 *
 * 归一化后的条目与调度都是纯数据且键序固定，因此同一张表永远得到同一个值；
 * 任何一层（内置、用户文件、插件配置）变了都会变。
 * @param entries - 归一化后的条目数组。
 * @param schedules - 合并后的命名调度表。
 * @returns 32 位无符号整数。
 */
function tableRevision(entries, schedules) {
  return createHash('sha256').update(JSON.stringify({ entries, schedules })).digest().readUInt32BE(0)
}
...
export class PricingStore {
  ...
  constructor(config) {
    ...
    this.fileMtimeMs = -1
    this.fileError = null
    /** 有效表的内容指纹；每次重建时重算，变了就通知监听者。 */
    this.revision = 0
    this.revisionListeners = new Set()
    ...
    this.revision = tableRevision(this.entries, this.schedules)
  }

  /**
   * 订阅「有效价目表变了」。
   *
   * 投影的 `stateVersion` 带着表指纹，价目表一变就重新注册，持久化检查点因此版本
   * 不匹配，整段日志会按新表重折——历史样本的时段归属跟着新规则走。
   * @param listener - 表内容变化时调用。
   * @returns 退订函数。
   */
  onChange(listener) {
  ...
  /**
   * 投影状态版本：折叠语义版本与有效价目表指纹的组合。
   *
   * 折叠会把「这个样本属于哪个时段」写进状态，而那是一个纯粹由价目表决定的派生
   * 值。把它烘进持久化检查点意味着改规则/补价格都改不动历史；把表指纹并进
   * `stateVersion` 之后，检查点随表失效，历史因此能自己算对。
   *
   * 上限约 7.3e9，仍是安全整数。
   * @returns 可直接用作 `stateVersion` 的整数。
   */
  projectionVersion() {
    return FOLD_VERSION * REVISION_SCALE + this.revision
  }
  ...
  rebuild() {
    ...
    const next = tableRevision(this.entries, this.schedules)
...
export function createProjectionDefinition(store) {
  return {
    key: 'tokenFee',
    stateVersion: store.projectionVersion(),
    stateSchema: {
      parse(value) {
        ...
        if (rawRows !== null && typeof rawRows === 'object' && !Array.isArray(rawRows)) {
          for (const [key, row] of Object.entries(rawRows)) {
            ...
            rows[key] = {
              ...
              tariff: row.tariff === 'offPeak' ? 'offPeak' : row.tariff === 'flat' ? 'flat' : 'peak',
              ...buckets,
            }
          }
        }
    ...
    apply: (state, event) => {
      ...
      // 兜底是 `'flat'` 而不是 `'peak'`：条目还不存在或只有统一单价时，两档价格在
      // 文件里根本不存在，把「不知道」记成高峰会让高峰用量凭空变大。
      const tariff = tariffOf(entry, event.time)
...
export function apply(ctx, config) {
  ...
  ctx.inject(['sessionProjections'], (scoped) => {
     ...
     * 安装（或重装）投影注册。
     *
     * 折叠会把「这个样本属于哪个时段」写进状态，而那是纯粹由价目表决定的派生值。
     * 注册时用的 `stateVersion` 带着表指纹，因此价目表一变就重装一次：旧注册先
     * 撤下，检查点的 `ver` 随即不匹配，DSH 会把整段日志按新表从 `seq 0` 重折，
     * 历史样本的时段归属因此跟着新规则走，而不是冻结在当初那次折叠的判定上。
     */
    let dispose = null
    const install = () => {
      dispose?.()
      dispose = scoped.effect(
        () => scoped.sessionProjections.register(createProjectionDefinition(store)),
        'token-fee: tokenFee projection',
      )
    }
    install()
    scoped.effect(() => store.onChange(install), 'token-fee: pricing revision reload')
  })
