...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			".tf_sep{color:var(--dsw-alias-label-dimmed);margin:0 6px}",
			...
			".tf_clock{box-sizing:border-box;width:38px;height:24px;padding:0;border:1px solid var(--dsw-alias-border-l2);border-radius:6px;background:var(--dsw-alias-bg-overlay,var(--dsw-alias-bg-base));color:var(--dsw-alias-label-primary);font:inherit;font-size:12px;line-height:18px;text-align:center;font-variant-numeric:tabular-nums}",
			".tf_clock:focus{outline:none;border-color:var(--dsw-alias-border-l1)}",
			".tf_clock[data-invalid]{border-color:var(--dsw-alias-state-error-primary)}",
			...
			".tf_ruleName{box-sizing:border-box;flex:1;min-width:0;max-width:240px;height:24px;padding:0 7px;border:1px solid var(--dsw-alias-border-l2);border-radius:7px;background:var(--dsw-alias-bg-overlay,var(--dsw-alias-bg-base));color:var(--dsw-alias-label-primary);font:inherit;font-size:12px;font-weight:600;line-height:18px}",
			".tf_ruleName:focus{outline:none;border-color:var(--dsw-alias-border-l1)}",
			".tf_ruleName[data-invalid]{border-color:var(--dsw-alias-state-error-primary)}",
			...
			// 保存条常驻顶部且吸顶，内容会从它下面滚过，因此必须用与面板一致的
			// 材质：同一个 `--dsw-specific-menu` 底色，并同样配上毛玻璃滤镜
			// （见 .tf_panel 的说明）。0.1.7-alpha.1 起该底色是半透明的，少了滤镜
			// 这条就会透出滚过去的正文；更早的版本底色本就不透明，滤镜失效也无妨。
			".tf_saveBar{display:flex;align-items:center;gap:6px;flex-wrap:wrap;flex:none;padding:8px 10px;margin-bottom:10px;border:1px solid var(--dsw-alias-border-l2);border-radius:10px;background:var(--dsw-specific-menu);backdrop-filter:var(--dsw-menu-backdrop-filter);position:sticky;top:0;z-index:2}",
				...
				el("div", { className: S.entryHead },
					builtin
						...
						: el("input", {
							...
							value: draftName,
							"data-invalid": nameProblem === null ? undefined : "true",
