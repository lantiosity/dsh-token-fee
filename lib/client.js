...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const css = [
			...
			// composer dock 的多个 occupant 默认是 InputBar 纵向弹性布局里的独立行，
			// 因此费用胶囊会被内置的统计胶囊挤到第二行。把二者的共同父级改成可换行
			// 的横向布局，并让非胶囊子元素（输入卡片、提示条等）继续独占整行；
			// 只有在两种胶囊同时挂载时才生效，单独挂载时不受影响。
			":has(> [data-composer-stats]):has(> .tf_root){display:flex;flex-direction:row;flex-wrap:wrap;align-items:center;justify-content:center;gap:0 4px}",
			":has(> [data-composer-stats]):has(> .tf_root)>*{flex:none}",
			":has(> [data-composer-stats]):has(> .tf_root)>[data-composer-stats]{flex:none;width:auto}",
			":has(> [data-composer-stats]):has(> .tf_root)>*:not([data-composer-stats]):not(.tf_root){flex:0 0 100%}",
		].join("");
