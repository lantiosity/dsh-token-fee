...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * 统一的元素构造器：`jsxs` 能同时处理单个子节点与子节点数组，
		 * 因此手写调用不需要区分 `jsx` 与 `jsxs`。
		 */
		...
		/** 打开期间监听外部点击与 Escape。 */
		function useDismiss(open, onClose, rootRef) {
			useEffect(() => {
				if (!open) return;
		...
		function PricingEditor({ pricing, t }) {
			...
			useEffect(() => {
				...
				setEntries((data.file.entries ?? []).map(cloneEntry));
				setSchedules({ ...(data.file.schedules ?? {}) });
			}, [data]);
			...
			const save = async () => {
				...
				try {
					...
					setEntries((body.file.entries ?? []).map(cloneEntry));
			...
			return el("div", null,
				...
				el("div", { className: S.section },
					...
					(data.builtin ?? []).map((entry) => el("div", { key: entry.id, className: S.card },
						...
						el("div", { className: S.rows2 },
							BUCKETS.map((bucket) => el("span", { key: bucket + "-l", className: S.rowLabel }, t("bucket." + bucket))),
							BUCKETS.map((bucket) => el("span", { key: bucket + "-c", className: S.rowCost },
								formatUnitPrice(entry.prices.peak[bucket] ?? 0, entry.currency))),
						),
						entry.prices.offPeak === undefined
							? null
							: el("div", { className: S.rows2, style: { marginTop: "4px" } },
								BUCKETS.map((bucket) => el("span", { key: bucket + "-ol", className: S.rowLabel }, t("editor.offPeakShort", { bucket: t("bucket." + bucket) }))),
								BUCKETS.map((bucket) => el("span", { key: bucket + "-oc", className: S.rowCost },
									formatUnitPrice(entry.prices.offPeak[bucket] ?? 0, entry.currency))),
							),
