...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/**
		 * provider / model 输入框：可直接输入，也可从当前已配置的模型中选择。
		 *
		 * 用原生 `datalist` 提供建议而不是下拉框：adapter 允许未列出的模型 id，
		 * 因此输入必须保持自由，建议只是方便。
		 */
		function RouteField({ label, value, onChange, options, placeholder, listId }) {
			return el("label", { className: S.field },
				el("span", { className: S.fieldLabel }, label),
				el("input", {
					className: S.input,
				...
				el("datalist", { id: listId },
					options.map(option => el("option", { key: option, value: option }))),
			);
		}
						...
						return {
							...
							prices: {
								...
								...(entry.prices.offPeak === undefined
									? {}
									: { offPeak: toPriceBlock(entry.prices.offPeak, label, t) }),
							},
