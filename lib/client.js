...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		 ...
		 * 规则选回「不区分」、删掉一条仍被引用的规则。不先拦下的话，用户只能看到
		 * host 抛出的技术报错。
		 ...
		 */
		function collectProblems(entries, scheduleNames, t) {
			const problems = [];
		...
		function scheduleSummary(schedule, t) {
			const windows = schedule.peakWindows.map(window => window[0] + "–" + window[1]).join(t("editor.listSeparator"));
		...
		function PricingEditor({ pricing, t, onOverlayToggle }) {
			...
			const save = async () => {
				...
				const problems = collectProblems(entries, scheduleNames, t);
