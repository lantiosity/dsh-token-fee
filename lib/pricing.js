...
export function normalizeEntry(raw, label = 'pricing entry', schedules = BUILTIN_SCHEDULES) {
  ...
  const currency = nonEmptyString(raw.currency) ?? DEFAULT_CURRENCY
  if (!/^[A-Za-z]{3}$/.test(currency)) throw new Error(`${label}.currency 必须是三位货币代码`)
...
/** 把若干用量行按 (provider, model, tariff) 聚合，供展示前合并同模型的多时段。 */
export function aggregateRows(rows) {
  const byKey = new Map()
  for (const row of rows ?? []) {
    const tariff = row.tariff ?? 'peak'
    const key = `${row.provider}\u0000${row.model}\u0000${tariff}`
    const previous = byKey.get(key)
    byKey.set(key, {
      provider: row.provider,
      model: row.model,
      tariff,
      ...addBuckets(previous ?? zeroBuckets(), row),
    })
  }
  return [...byKey.values()]
}
