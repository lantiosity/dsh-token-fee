 ...
 *
 * `schedule` 写成命名引用 `'deepseek'` 而不是内联一份规则：内置条目在每次重建
 * 有效表时都会按当前命名表重新解析，用户在内置规则上做的改动（例如官方调整了
 * 高峰时段）因此能作用到内置条目；内联对象会让内置条目永远锁死在出厂规则上。
 */
 ...
 * 把调度时区下的墙上时间换算成绝对时间戳。
 *
 * 先假设墙上时间就是 UTC 并据此修正一次，再用修正后的时刻复核偏移：夏令时切换
 * 当天两个偏移不同，第二次的结果才是正确的那一侧。
 * @param wall - `{ year, month, day, hour, minute, second }`，均为时区本地值。
 ...
 * 找出一个时刻之后的下一次时段切换。
 *
 * 时段只可能在窗口起点、窗口终点与零点这三类边界上变化，因此只需枚举这些
 * 边界：从当天起逐日推进，取第一个判定结果与当前不同的边界。逐日推进的上限
 * 取 8 天——最稀疏的规则（例如只在周一 09:00-12:00 计高峰）从周一 12:00 起
 * 要等将近 7 天才会再次切换。
 * @param entry - 归一化后的价目条目（必须带调度与空闲价）。
 * @param timestamp - 起始时刻（毫秒）。
 * @returns `{ at, tariff }`；找不到时为 null。
 */
function nextTariffBoundary(entry, timestamp) {
  ...
  for (const [start, end] of schedule.peakWindows) {
    boundaries.add(minuteOf(String(start)))
    boundaries.add(minuteOf(String(end)))
  }
...
/**
 * 判定一个条目在某一时刻的计费状态，含下一次切换的等待时间。
 *
 * host 与浏览器半各有一份同构实现：host 用 {@link tariffAt} 给每个用量样本定档，
 * 浏览器用这里的 `remainingMs` 渲染胶囊上的「当前时段 + 剩余时间」。两侧必须
 * 给出相同的时段与相同的切换时刻，否则倒计时会指向一个不会发生切换的时间点。
 * @param entry - 归一化后的价目条目。
 * @param timestamp - 当前时刻（毫秒）。
 * @returns `{ split, tariff, nextTariff, remainingMs }`；不分峰谷时 `split` 为
 *   false，`nextTariff` 与 `remainingMs` 为 null。
 */
export function tariffStateAt(entry, timestamp) {
  if (entry.prices.offPeak === undefined || entry.schedule === null) {
 ...
 * 折叠一个用量样本时的时段归属。
 *
 * 这是**唯一**应该用来给样本分桶的入口：找不到价目条目、或条目没有空闲价时返回
 * `'flat'`（不分峰谷），而不是冒充 `'peak'`。后者会让「高峰用量」凭空变大——条目
 * 还不存在（用户尚未配置）或只填了统一单价时，那两档价格在文件里根本不存在。
 *
 * 投影状态是持久化的检查点，标签一旦写进去就冻结，因此把「不知道」说成「高峰」
 * 无法靠事后补规则自动纠正；调用方另需让检查点随价目表失效（见 `stateVersion`）。
 * @param entry - 命中的价目条目，未命中为 null。
 * @param timestamp - 样本时间（毫秒）。
 * @returns `'peak'`、`'offPeak'` 或 `'flat'`。
 */
export function tariffOf(entry, timestamp) {
  if (entry === null || entry === undefined) return 'flat'
  if (entry.prices.offPeak === undefined || entry.schedule === null) return 'flat'
  // 时间戳不可用时同样不知道落在哪个时段，不能顺手记成高峰。
 ...
 * @param entry - 命中的价目条目。
 * @param tariff - `'peak'`、`'offPeak'` 或 `'flat'`；只有 `'offPeak'` 走空闲价块。
...
export function computeCost(rows, entries, displayCurrency = DEFAULT_CURRENCY) {
  ...
  for (const row of rows ?? []) {
    ...
    const cost = costOfEntry(entry, row.tariff ?? tariffOf(entry, row.time), buckets)
